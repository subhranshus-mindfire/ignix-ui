...
const AnimatedTextarea: React.FC<AnimatedTextareaProps> = ({
  ...
  useEffect(() => {
    if ((variant === 'expandable' || variant === 'smoothExpand') && textareaRef.current) {
    ...
    >
      ...
      <div className="relative">
        <motion.textarea
          ...
          style={{
            height: variant === 'expandable' || variant === 'smoothExpand' ? height : 'auto',
            minHeight: '100px',
          }}
          onFocus={handleFocus}
          onBlur={handleBlur}
...
const textareaVariants: Record<string, TextareaVariant> = {
  ...
  neonGlow: {
    ...
    textarea: {
      ...
      animate: { boxShadow: '0 0 15px rgba(59, 130, 246, 0.5), 0 0 30px rgba(59, 130, 246, 0.3)' },
    },
  },
  particleField: {
    label: {
  ...
  wave: {
    ...
    textarea: {
      initial: { y: 0 },
      animate: { y: [0, -2, 2, 0], transition: { repeat: Infinity, duration: 2 } },
    },
  },
  spotlight: {
    label: {
  ...
  cosmic: {
    ...
    textarea: {
      initial: { scale: 1 },
      animate: { scale: [1, 1.02, 0.98, 1], transition: { repeat: Infinity, duration: 3 } },
    },
  },
  hologram: {
    label: {
      initial: { y: 0, opacity: 0.6 },
      animate: { y: -24, opacity: [0.6, 1, 0.6], transition: { repeat: Infinity, duration: 2 } },
    },
    textarea: {
      initial: { opacity: 0.8 },
