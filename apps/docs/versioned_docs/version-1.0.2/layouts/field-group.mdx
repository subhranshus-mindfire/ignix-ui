---
sidebar_position: 9
title: FieldGroup
description: A form layout component for grouping related fields under a common title with optional border and flexible layouts.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import FieldGroup from '@site/src/components/UI/field-group';
import FieldGroupDemo from '@site/src/components/Demo/FieldGroupDemo';

## Overview

The **FieldGroup** component is used inside forms to visually group related fields under a common title.  
It improves structure, accessibility, and scannability of complex forms.

---

## Preview

<Tabs defaultValue="preview">
  <TabItem value="preview" label="Preview" default>
    <div className="p-4 border rounded-lg">
      <FieldGroup title="Personal Information" columns="auto" spacing="normal" border={true}>
        <div className="grid gap-2">
          <input type="text" placeholder="First Name" className="border p-2 rounded" />
          <input type="text" placeholder="Last Name" className="border p-2 rounded" />
        </div>
      </FieldGroup>
    </div>
  </TabItem>

  <TabItem value="code" label="Code">
    ```tsx
    import FieldGroup from './components/ui/field-group';

    export default function Demo() {
      return (
        <FieldGroup title="Personal Information" columns="auto" spacing="normal" border>
          <div className="grid gap-2">
            <input type="text" placeholder="First Name" />
            <input type="text" placeholder="Last Name" />
          </div>
        </FieldGroup>
      );
    }
    ```
  </TabItem>
</Tabs>

---

## Installation

<Tabs>
  <TabItem value="npm" label="npm" default>
    ```bash
    npx @mindfiredigital/ignix-ui add field-group
    ```
  </TabItem>

  <TabItem value="yarn" label="Yarn">
    ```bash
    yarn @mindfiredigital/ignix-ui add field-group
    ```
  </TabItem>

  <TabItem value="pnpm" label="pnpm">
    ```bash
    pnpm @mindfiredigital/ignix-ui add field-group
    ```
  </TabItem>

  <TabItem value="manual" label="manual">
    ```tsx
    // Minimal FieldGroup example (manual)
    import React from 'react';
    import FieldGroup from './components/ui/field-group';

    export default function FieldGroupManualDemo() {
      return (
        <FieldGroup title="Shipping Address" border spacing="normal" columns="auto">
          <div className="grid gap-2">
            <input type="text" placeholder="Street" />
            <input type="text" placeholder="City" />
          </div>
        </FieldGroup>
      );
    }
    ```
  </TabItem>
</Tabs>

---

## Variants

<FieldGroupDemo />

---

## Usage Examples

### Grouped Form Fields

```tsx
<FieldGroup title="Login Details" border columns="auto">
  <input type="email" placeholder="Email Address" />
  <input type="password" placeholder="Password" />
</FieldGroup>
```

---

### Without Border

```tsx
<FieldGroup title="Preferences" border={false} columns="2">
  <label><input type="checkbox" /> Receive Newsletter</label>
  <label><input type="checkbox" /> Enable Dark Mode</label>
</FieldGroup>
```

---

## Props

| Prop       | Type                                | Default   | Description |
|------------|-------------------------------------|-----------|-------------|
| `title`    | `string`                            | `null`    | Title of the group, rendered inside a `<legend>` |
| `columns`  | `"auto" \| "1" \| "2" \| "3" ...`   | `"auto"`  | Defines layout for fields (auto, fixed grid) |
| `spacing`  | `"none" \| "tight" \| "normal" \| "loose"` | `"normal"` | Controls spacing between fields |
| `border`   | `boolean`                           | `true`    | Toggles fieldset border |
| `children` | `ReactNode`                         | —         | Form fields inside the group |
